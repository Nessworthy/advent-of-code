...
class Day07PartB implements Solution {
    ...
    private function calculate(int $total, int $current, array $numbers, Output $output, array $debug = []): bool {
        ...
        $combined = toInt($current . $num);
        $addDebug = array_merge($debug, ['+ ' . $num]);
        $multDebug = array_merge($debug, ['* ' . $num]);
        $combinedDebug = array_merge($debug, ['|| ' . $num]);

        return (
            ($multiplied <= $total && $this->calculate($total, $multiplied, $numbers, $output, $multDebug))
            || ($added <= $total && $this->calculate($total, $added, $numbers, $output, $addDebug))
            || ($combined <= $total && $this->calculate($total, $combined, $numbers, $output, $combinedDebug))
        );
    }
}
