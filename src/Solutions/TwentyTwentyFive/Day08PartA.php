...
class Day08PartA implements Solution {
    ...
    {
        $connectionLimit = $this->context->isTestRun() ? 10 : 1000;
        ...
        usort($pointCircuits, static function ($a, $b) {
            return count($b) <=> count($a);
        });

        $total = 0;
        foreach (array_slice($pointCircuits, 0, 3, true) as $id => $circuit) {
            $output->writeLine(sprintf('Circuit %d has %d points', $id, count($circuit)));
            if ($total === 0) {
                $total = count($circuit);
            } else {
                $total *= count($circuit);
